...
class MoleculeUnitTest : public CxxTest::TestSuite {
  ...
  void performAssertionsOnPiElectronSystem(const ElectronSystem* electronSystem) {
    TS_ASSERT_EQUALS(electronSystem->numAtoms(), 2);
    TS_ASSERT(electronSystem->atoms().contains(atomA));
    TS_ASSERT(electronSystem->atoms().contains(atomB));
    TS_ASSERT_EQUALS(electronSystem->numElectrons(), 2);
...
public:
  ...
  void testNoPiElectronSystemForSingleBond() {
    ...
    molecule->updateElectronSystems();
    TS_ASSERT_EQUALS(molecule->m_electronSystems.size(), 0);
  }
